...
export function QueueStatus() {
  ...
  // Calculate stats from orders data - filter user orders first
  const userOrders = orders.filter((order) => order.userId === user?.id);
  const stats = {
    pending: userOrders.filter((order) => order.status === "submitted").length,
    inProgress: userOrders.filter(
      (order) => order.status === "approved" || order.status === "started"
    ).length,
    completed: userOrders.filter((order) => order.status === "finished").length,
    failed: userOrders.filter(
    ...
    <div className="space-y-8">
      <div className="mb-8">
        ...
        <p className="text-gray-400">
          Track the status of your submitted print requests.
        </p>
      </div>
      ...
      <Card className="bg-slate-800 border-slate-700">
        <CardHeader>
          ...
          <CardDescription>
            Your print requests and their current status
          </CardDescription>
        </CardHeader>
        <CardContent>
          {orders.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-gray-400 mb-4">
                Submit your first 3D print request to get started!
              </p>
