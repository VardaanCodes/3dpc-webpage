...
requiredEnvVars.forEach(varName => {
  ...
  const isSet = value && value.trim() !== '' && !value.includes('your_') && !value.includes('placeholder');
...
console.log('\n📋 Summary:');
console.log(`✅ Configured: ${Object.values(envStatus).filter(s => s.set).length}`);
console.log(`❌ Missing: ${Object.values(envStatus).filter(s => !s.set).length}`);
...
if (hasErrors) {
  ...
  console.log('3. Set the missing environment variables with your actual Firebase config values');
  console.log('4. You can find these values in your Firebase console under Project Settings');
